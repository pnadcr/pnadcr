...
#' @return gt table.
#'
#' @examples \donttest{pnadc_percent(~V3003A, ~UF + V2007, 2019, 1)}
pnadc_percent <- function(variable, by, year, quartile, path = FALSE, export = FALSE) {
  ...
  create_table <- function(variable, by, year, quartile, tabela) {
    ...
    tabela_final <- tabela_final %>%
      ...
      gt::fmt_percent(columns = 3:ncol(tabela))
  ...
  save_table <- function(tabela_final, path, year, quartile, export) {
    if (export == "csv") {
      utils::write.csv2(
        tabela_final,
        file = fs::path_home(paste("Tabela_pnadc_porcent_", year, "_", quartile, ".csv", sep = "")),
        fileEncoding = "UTF-8",
        row.names = FALSE
      )
